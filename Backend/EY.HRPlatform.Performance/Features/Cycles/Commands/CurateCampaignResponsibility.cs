...
public sealed class CurateCampaignResponsibilityCommandHandler(
    ...
    ICoreWorkforceClient workforceClient) : ICommandHandler<CurateCampaignResponsibilityCommand, Result>
{
    public async Task<Result> Handle(CurateCampaignResponsibilityCommand request, CancellationToken cancellationToken)
    {
        ...
        if (cycle.Status != PerformanceCycleStatus.AssignmentPreparation)
            return Result.Failure(Error.Conflict("Cycle.NotInPreparation", "Responsibilities can only be curated during assignment preparation."));
        if (!cycle.Participants.Any(x => x.EmployeeId == request.SubjectEmployeeId))
            return Result.Failure(Error.Validation("Cycle.UnknownSubject", "The responsibility subject is not in this campaign population."));
        ...
        if (subject is null)
            return Result.Failure(Error.Validation("Cycle.IneligibleSubject", "The responsibility subject is inactive or outside the current workforce scope."));

        var assignee = currentWorkforce.SingleOrDefault(x => x.EmployeeId == request.AssigneeEmployeeId && x.IsActive);
        if (assignee is null)
            return Result.Failure(Error.Validation("Cycle.IneligibleAssignee", "The selected assignee is inactive or outside the current workforce scope."));
        ...
        {
            return Result.Failure(Error.Validation("Cycle.InvalidResponsibility", exception.Message));
        }
