...
public class MarkCycleReadyToLaunchCommandHandlerTests
{
    ...
    public async Task Handle_WhenEveryParticipantCoveredAndDeltaAccepted_MarksCampaignReady()
    {
        ...
        var workforce = new FakeCoreWorkforceClient
        {
            CampaignWorkforceContext = new(
                ...
                "baseline",
                [
                    CampaignMember(subjectId, isActive: true, primaryManagerEmployeeId: assigneeId),
                    CampaignMember(assigneeId, isActive: true)
                ])
        };
    ...
    public async Task Handle_WhenFinalApproverNoLongerActive_BlocksLaunch()
    {
        ...
        var workforce = new FakeCoreWorkforceClient
        {
            CampaignWorkforceContext = new(
                ...
                [
                    CampaignMember(subjectId, isActive: true, primaryManagerEmployeeId: assigneeId),
                    CampaignMember(assigneeId, isActive: false)
                ])
        };
    ...
    [Fact]
    public async Task Handle_WhenPrimaryManagerResponsibilityNoLongerMatchesCurrentManager_BlocksLaunch()
    {
        var tenantId = Guid.NewGuid();
        var now = DateTime.UtcNow;
        await using var db = PerformanceTestContext.Create(tenantId, out var tenantContext);
        ...
        var currentPrimaryManagerId = Guid.NewGuid();
        var workforce = new FakeCoreWorkforceClient
        {
            CampaignWorkforceContext = new(
                now,
                "baseline",
                [
                    CampaignMember(subjectId, isActive: true, primaryManagerEmployeeId: currentPrimaryManagerId),
                    CampaignMember(assigneeId, isActive: true),
                    CampaignMember(currentPrimaryManagerId, isActive: true)
                ])
        };
