...
public sealed class GetEmployeeReportingLinesByKeyQueryHandler(
    CoreHRDbContext dbContext,
    IEmployeeReadModelPolicy employeeReadModelPolicy,
    ITenantSettingsReadService tenantSettingsReadService) : IQueryHandler<GetEmployeeReportingLinesByKeyQuery, Result<EmployeeReportingLinesDto>>
{
    public async Task<Result<EmployeeReportingLinesDto>> Handle(
        GetEmployeeReportingLinesByKeyQuery request,
        CancellationToken cancellationToken)
    {
        var settings = await tenantSettingsReadService.GetCurrentAsync(cancellationToken);
        var employee = await LoadEmployeeByKeyAsync(request.EmployeeKey, cancellationToken);
        if (employee is null)
        {
            return Result.Failure<EmployeeReportingLinesDto>(Error.NotFound("Employee", Guid.Empty));
        }
    ...
    private Task<Employee?> LoadEmployeeByKeyAsync(string employeeKey, CancellationToken cancellationToken)
        => dbContext.Employees
            .AsNoTracking()
            .Include(e => e.Manager)
            .Include(e => e.OrgUnit)
            .FirstOrDefaultAsync(e => e.StableEmployeeKey == employeeKey, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var manager = await LoadEmployeeByIdAsync(currentManagerId.Value, cancellationToken);
            if (manager is null)
            {
                break;
            }
    ...
    private Task<Employee?> LoadEmployeeByIdAsync(
        Guid employeeId,
        CancellationToken cancellationToken)
        => dbContext.Employees
            .AsNoTracking()
