...
export function OrganizationDetailView({ org }: { org: Organization }) {
  ...
  const onCopyLink = useCallback(async () => {
    ...
    const path = `/core/corehr/invite/accept?org=${encodeURIComponent(org.name)}&email=${encodeURIComponent(org.primaryAdminEmail ?? "")}`;
  ...
  return (
    <main className="mx-auto w-full max-w-7xl bg-ch-surface p-8 font-chBody text-ch-on-surface">
      {actionBanner && (
        <p
          className="mb-6 rounded-lg bg-ch-surface-container-low px-4 py-2 text-sm font-medium text-ch-on-surface"
          role="status"
        >
          {actionBanner}
        </p>
