...
import type { MappingStatus } from "../model/match-mapping";
...
/**
 * Edits the Mapping Plan one row at a time. The chosen value shows at once while the server
 * settles it; the response is the new truth. A failure keeps the last authoritative value and
 * says so briefly. Edits are serialized so each one lands on the version it was made against.
 */
export function useMatchEdit() {
  ...
  async function edit(
    key: string,
    value: string,
    change: Omit<Parameters<typeof updateMatch.mutateAsync>[0], "id" | "version">
  ) {
    if (pending) return;
    setPending({ key, value });
    try {
      await updateMatch.mutateAsync({ id: session.id, version: session.version, ...change });
      return true;
    } catch (error) {
      toast.error("That change wasn’t saved", {
