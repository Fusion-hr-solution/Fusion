...
import { usePeopleAccessStatus, usePeopleProfile, usePeopleTimeline } from "../api/use-people";
import {
  EmployeeIdentity,
  EmploymentStatus,
  Monogram,
...
function employmentLine(employment: PeopleProfileDto["employment"]): string {
  ...
  if (state === "Scheduled") return start ? `Starts ${formatWorkforceDate(start, { month: "long" })}` : "Starts later";
  if (state === "Former") {
  ...
  return start ? `Since ${formatWorkforceDate(start, { month: "long" })}` : "Active";
}
...
function Eyebrow({ children, className }: { children: React.ReactNode; className?: string }) {
  return <h2 className={cn("type-eyebrow text-muted-foreground", className)}>{children}</h2>;
}
...
function ChangeField({ field }: { field: PeopleChangeFieldDto }) {
  return (
    <p className="type-body">
      ...
      {field.from ? (
        <>
          <span className="text-muted-foreground line-through decoration-muted-foreground/40">{field.from}</span>
          <ArrowRight aria-hidden className="mx-1.5 inline size-3.5 -translate-y-px text-muted-foreground" />
        </>
      ) : null}
    ...
    <PageContainer width="wide" className="pb-16">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-10 grid gap-x-12 gap-y-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
          <div className="space-y-4"><Skeleton className="h-4 w-24" /><Skeleton className="h-24 w-full" /></div>
          <div className="space-y-4"><Skeleton className="h-4 w-24" /><Skeleton className="h-16 w-full" /></div>
        </div>
      </div>
    </PageContainer>
  );
...
export default function PeopleProfileWorkspace({ employeeKey }: { employeeKey: string }) {
  ...
  const access = usePeopleAccessStatus(employeeKey, Boolean(profile.data) && !asOfParam);
  ...
  return (
    <PageContainer width="wide" className="pb-16">
      <div className="mx-auto max-w-5xl">
        <Link href="/people" className="inline-flex items-center gap-2 text-sm text-muted-foreground underline-offset-4 hover:text-foreground hover:underline focus-visible:rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                ...
                {showResult && established ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-success/30 bg-success-subtle px-2.5 py-1 type-meta font-medium text-foreground motion-safe:animate-in motion-safe:fade-in">
                    <Check className="size-3.5 text-success" aria-hidden /> {established === "hire" ? "Hired" : "Added"}
                  </span>
                ) : null}
              </div>
              {work ? (
          ...
          <div className="flex flex-col items-end gap-3">
            {isAsOf ? (
              <div className="inline-flex items-center gap-2.5 rounded-full border border-primary/30 bg-primary/[0.06] py-1.5 pl-3.5 pr-1.5 motion-safe:animate-in motion-safe:fade-in">
                <span className="type-label">As of {formatWorkforceDate(employee.viewedDate, { month: "long" })}</span>
                <Button size="sm" variant="ghost" className="h-7 gap-1.5 rounded-full px-2.5 text-muted-foreground hover:text-foreground" onClick={returnToToday}>
            ...
            {showActions ? (
              <div className="flex items-center gap-2">
                <Button asChild size="sm"><Link href={`/people/${employeeKey}/change-work`}><BriefcaseBusiness className="size-4" /> Change work</Link></Button>
                <Button asChild size="sm" variant="outline"><Link href={`/people/${employeeKey}/change-manager`}>Change manager</Link></Button>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button size="icon" variant="ghost" aria-label="More actions"><MoreHorizontal className="size-4" /></Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem asChild>
                      <Link href={`/people/${employeeKey}/end-employment`} className="text-destructive focus:text-destructive">
          ...
          <section className="mt-8 motion-safe:animate-in motion-safe:fade-in">
            ...
            <ul className="mt-3.5 grid gap-3 sm:grid-cols-2">
              {upcoming.map((item, index) => (
                <UpcomingCard key={`${item.kind}-${item.effectiveDate}-${index}`} item={item} onView={() => goAsOf(isoDay(item.effectiveDate))} />
              ))}
            </ul>
          </section>
        ) : null}
        ...
        <div className="mt-10 grid gap-x-12 gap-y-10 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] lg:items-start">
          <div className="order-2 min-w-0 lg:order-1">
            <TimelineSection events={timeline.data?.timeline} loading={timeline.isLoading} />
          </div>
            ...
            {employee.directReportCount > 0 ? (
              <section>
                ...
                <ul className="mt-3.5 space-y-3.5">
                  ...
                  {employee.directReportCount > employee.directReports.length ? (
                    <li className="pl-11 type-meta text-muted-foreground">
                      +{employee.directReportCount - employee.directReports.length} more
                    </li>
                  ) : null}
                </ul>
              </section>
    ...
    >
      <div className="grid lg:grid-cols-[1.5fr_minmax(0,1fr)]">
        <div className="p-6">
          ...
          {work ? (
            <div className="mt-3">
              <p className="type-section-title text-foreground">{work.jobTitle}</p>
              <div className="mt-1.5">
                <OrgPath name={work.organizationName} path={work.organizationPath} unitClassName="type-panel-title text-foreground" />
              </div>
            ...
            <p className="mt-3 type-body text-muted-foreground">Work details unavailable on this date.</p>
          )}
        </div>
        ...
        <div className="grid divide-y border-t lg:border-l lg:border-t-0">
          <div className="p-6">
            ...
            <div className="mt-3">
              {primaryManager ? (
                <EmployeeIdentity name={primaryManager.displayName} employeeNumber={primaryManager.employeeNumber} href={`/people/${primaryManager.employeeKey}`} size="sm" />
              ) : (
          ...
          <div className="p-6">
            ...
            <div className="mt-3 flex items-center gap-2.5">
              <EmploymentStatus state={employment.state} />
              <span className="type-body text-muted-foreground">{employmentLine(employment)}</span>
            </div>
            {employment.employmentType ? <p className="mt-1.5 type-meta text-muted-foreground">{employment.employmentType}</p> : null}
          </div>
        </div>
      </div>
    </section>
...
function AsOfControl({ today, onPick }: { today: string; onPick: (date: string) => void }) {
  return (
...
function UpcomingCard({ item, onView }: { item: PeopleUpcomingChangeDto; onView: () => void }) {
    ...
    <li className="group flex flex-col justify-between gap-3 rounded-xl border bg-card p-4 transition-colors hover:border-primary/30 motion-reduce:transition-none">
      <div className="min-w-0">
        ...
        <div className="mt-2 space-y-0.5">
          {item.fields.map((field) => <ChangeField key={field.label} field={field} />)}
        </div>
      </div>
      <Button size="sm" variant="ghost" className="-ml-2 self-start text-muted-foreground group-hover:text-foreground" onClick={onView}>
        View as of {formatWorkforceDate(item.effectiveDate)} <ChevronRight className="size-4" />
      </Button>
    </li>
  );
}

function TimelineSection({ events, loading }: { events?: PeopleTimelineEventDto[]; loading: boolean }) {
  if (loading) {
    return (
      <section>
        <Eyebrow>History</Eyebrow>
        <div className="mt-4 space-y-3"><Skeleton className="h-10 w-full" /><Skeleton className="h-10 w-2/3" /></div>
      </section>
    );
  }
    ...
    <section>
      ...
      <ol className="mt-4">
        {events.map((event, index) => (
          <li key={`${event.kind}-${event.effectiveDate}-${index}`} className="flex gap-4">
            <span className={cn(
            ...
            <div className={cn("min-w-0 pt-0.5", index < last && "pb-6")}>
              ...
              {event.fields.length > 0 ? (
                <div className="mt-1 space-y-0.5">
                  {event.fields.map((field) => <ChangeField key={field.label} field={field} />)}
                </div>
              ) : null}
            </div>
          </li>
