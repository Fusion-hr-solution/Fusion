...
const { mockPush, mockReplace, mockSearchParams, mockUsePeople, mockUseOrg, mockUseAuth, mockCanManage, mockCanImport } =
  vi.hoisted(() => ({
    mockPush: vi.fn(),
    mockReplace: vi.fn(),
    ...
    mockUseAuth: vi.fn(() => ({ user: { id: "u1" }, isAuthenticated: true, isLoading: false })),
...
function page(items: PeopleRowDto[], totalCount = items.length): PeoplePageDto {
  return { items, totalCount, page: 1, pageSize: 25, totalPages: Math.max(1, Math.ceil(totalCount / 25)) };
}
...
describe("PeopleWorkspace", () => {
  ...
  it("shows a true-empty workforce that owns the page, without roster query chrome", () => {
    ...
    render(<PeopleWorkspace />);
    expect(screen.getByText("Bring your people into Fusion")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Import workforce/i })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Add existing employee/i })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Hire employee/i })).toBeInTheDocument();
  ...
  it("surfaces a retry affordance on load failure", async () => {
    const refetch = vi.fn();
    mockUsePeople.mockReturnValue(queryResult({ error: new Error("Network down"), refetch }));
    render(<PeopleWorkspace />);
  ...
  it("uses explicit language for scheduled, former and incomplete employment, and no manager", () => {
    mockUsePeople.mockReturnValue(
      queryResult({
        data: page([
          row({ employeeKey: "E-S", displayName: "Sched Person", employmentState: "Scheduled", employmentStart: "2999-01-01T00:00:00Z", primaryManager: null }),
          row({ employeeKey: "E-F", displayName: "Former Person", employmentState: "Former", employmentEnd: "2020-01-01T00:00:00Z" }),
          row({ employeeKey: "E-I", displayName: "Incomplete Person", employmentState: "Incomplete", work: null, primaryManager: null }),
        ]),
      }),
    );
    render(<PeopleWorkspace />);
    ...
    expect(screen.getAllByText("Employment unavailable").length).toBeGreaterThan(0);
    expect(screen.getAllByText("Work details unavailable").length).toBeGreaterThan(0);
