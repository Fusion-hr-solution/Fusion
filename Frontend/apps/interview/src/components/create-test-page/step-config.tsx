...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-zinc-400">{children}</p>;
}
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="ml-9 text-[13px] text-zinc-500">Customize how candidates experience and complete this test</p>
      </div>
      ...
      <div className="space-y-5">
        ...
        <Card icon={MonitorPlay} title="Candidate Experience" description="Control what candidates see and do">
          <SwitchRow label="Allow question skipping" helper="Candidates can navigate back to unanswered questions"
            checked={config.allowSkipping} onChange={(v) => updateConfig({ allowSkipping: v })} />
          <SwitchRow label="Show progress bar" helper="Displays a step indicator during the test"
            checked={config.showProgressBar} onChange={(v) => updateConfig({ showProgressBar: v })} />
          <SwitchRow label="Restrict copy/paste" helper="Disables clipboard on coding questions"
            checked={config.restrictCopyPaste} onChange={(v) => updateConfig({ restrictCopyPaste: v })} />
          <SwitchRow label="Enable proctoring" helper="Requires webcam — needs integration"
            checked={config.enableProctoring} onChange={(v) => updateConfig({ enableProctoring: v })}>
        ...
        <Card icon={Timer} title="Time & Attempts" description="Time limits and attempt restrictions">
          <SwitchRow label="Enable time limit" checked={config.enableTimeLimit}
            ...
            {config.enableTimeLimit && (
              <div className="mb-3 flex items-center gap-3 rounded-xl border border-zinc-100 bg-zinc-50 px-3.5 py-3">
                <input type="number" min={1} value={config.timeLimitMinutes}
                  onChange={(e) => updateConfig({ timeLimitMinutes: Number(e.target.value) })}
                  className="w-24 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-[13px] font-medium text-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900/10" />
                <span className="text-[13px] text-zinc-500">minutes</span>
                <span className="text-[11px] text-zinc-400">· Recommended: 60–90 min for technical tests</span>
              </div>
            )}
          </SwitchRow>
          ...
          <div className="flex items-center justify-between gap-4 py-3.5">
            <div>
              ...
              <p className="mt-0.5 text-[12px] text-zinc-400">0 = unlimited attempts</p>
            </div>
            <input type="number" min={0} value={config.maxAttempts}
              onChange={(e) => updateConfig({ maxAttempts: Number(e.target.value) })}
              className="w-20 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-[13px] font-medium text-zinc-900 text-right focus:outline-none focus:ring-2 focus:ring-zinc-900/10" />
          </div>

          <SwitchRow label="Randomize question order" helper="Each candidate sees questions in a different order"
            checked={config.randomizeOrder} onChange={(v) => updateConfig({ randomizeOrder: v })} />
        </Card>
          ...
          <div className="py-4 space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button key={opt.value} onClick={() => updateConfig({ accessType: opt.value })}
                    className={cn(
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={f.key}>
                    <p className="mb-1 text-[11px] text-zinc-500">{f.label}</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-[12px] text-zinc-400">After how many days the link stops working</p>
              </div>
              <div className="flex items-center gap-2">
                <input type="number" min={1} value={config.linkExpiry}
                  onChange={(e) => updateConfig({ linkExpiry: Number(e.target.value) })}
                  className="w-20 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-[13px] font-medium text-zinc-900 text-right focus:outline-none focus:ring-2 focus:ring-zinc-900/10" />
                <span className="text-[13px] text-zinc-500">days</span>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card icon={Award} title="Scoring" description="Pass thresholds and grading behaviour">
          <div className="flex items-center justify-between gap-4 py-3.5">
            ...
            <div className="flex items-center gap-1.5">
              <input type="number" min={0} max={100} value={config.passingThreshold}
                onChange={(e) => updateConfig({ passingThreshold: Number(e.target.value) })}
                className="w-20 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-[13px] font-medium text-zinc-900 text-right focus:outline-none focus:ring-2 focus:ring-zinc-900/10" />
              <span className="text-[13px] text-zinc-500">%</span>
            </div>
          </div>

          <SwitchRow label="Allow partial credit" helper="Applicable to multi-part and coding questions"
            checked={config.allowPartialCredit} onChange={(v) => updateConfig({ allowPartialCredit: v })} />
          ...
          <div className="flex items-center justify-between gap-4 py-3.5">
            ...
            <div className="relative">
              <select value={config.assignedReviewer}
                onChange={(e) => updateConfig({ assignedReviewer: e.target.value })}
                className="appearance-none rounded-xl border border-zinc-200 bg-white py-2 pl-3 pr-8 text-[13px] text-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900/10">
      ...
      <div className="mt-8 flex items-center justify-between border-t border-zinc-100 pt-6">
        ...
        <button onClick={nextStep}
          className="flex items-center gap-2 rounded-xl bg-zinc-900 px-6 py-2.5 text-[14px] font-semibold text-white shadow-sm hover:bg-zinc-800 transition-all duration-150 active:scale-[0.98]">
