...
export function OnSiteCoursesList({ courses, scheduledDate }: OnSiteCoursesListProps) {
  ...
  return (
    <div className="ey-animate-fade-up space-y-4" style={{ animationDelay: "280ms" }}>
      {scheduledDate && (
        <div className="flex items-center gap-3 rounded-xl border border-[hsl(var(--ey-blue-500))]/20 bg-[hsl(var(--ey-blue-500))]/10 p-4">
          <Calendar className="h-5 w-5 text-[hsl(var(--ey-blue-500))] shrink-0" />
          <div>
            <p className="text-sm font-semibold text-[hsl(var(--ey-blue-500))]">{t("scheduledSession")}</p>
      ...
      <div className="space-y-2">
        {sorted.map((course, index) => (
          ...
          >
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[hsl(var(--ey-red-500))]/10">
