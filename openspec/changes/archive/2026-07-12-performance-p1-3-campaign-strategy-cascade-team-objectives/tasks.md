...
- [x] 4.1 Add `@repo/auth` permission helpers for team-objective management and strategic view; add `Team objectives` sidebar entry + `/performance/team-objectives` routes and breadcrumbs (list → `[slug]` workspace); truthful empty states (no permission → no entry; permission but no frozen responsibility → product-language explanation)
- [x] 4.2 Add team-objective terminology to the single terminology source (workspace labels, creation flow, availability footnote wording — no lifecycle words) and API client + query hooks in the existing app pattern (content-area skeletons, reads never write, saved changes reflect without refresh)
- [x] 4.3 Build the campaign team-objectives workspace: strategy-to-translate (read-only), frozen scope summary (count, preview, org units, schedule context), and the manager's team-objective list; design the elevated version of the moment first (impeccable pass) — this is the manager's cascade cockpit, not an admin table
- [x] 4.4 Build the create/edit experience as one clear business action (choose strategic objective → describe → success criteria → measurement method as bounded choice from the frozen snapshot → save), plus delete with confirmation; validation preserves entered values; conflict and permission failures are distinct and recoverable; quiet schedule-gated availability note per spec

## 5. Frontend — cascade coverage for HR and Direction

- [x] 5.1 Add the cascade coverage surface to the launched campaign detail (`/performance/campaigns/[slug]`): coverage indicators, strategic objectives with/without coverage, managers with/without team objectives, HR read-only view of all team objectives; informational tone, no blocker framing, Draft campaigns show no coverage surface
- [x] 5.2 Add the `Strategy` sidebar entry gated by strategic view + `/performance/strategy` routes (launched campaign list → per-campaign strategy & coverage) reusing the same coverage surface component; breadcrumbs per spec; no admin affordances leak to Direction
- [x] 5.3 Impeccable pass over both doors: one product voice with the campaign workspace, coverage moment has presence (this is Direction's first dedicated surface), bold-vs-plain judged explicitly and recorded

## 6. Verification and completion gate

- [x] 6.1 Backend: `dotnet build` + full Performance test suite (221 pass); frontend: `pnpm --filter performance test` (53 pass), `pnpm lint`, `pnpm type-check` all green; `@repo/auth` (52) + `@repo/api` (88) tests pass
- [x] 6.2 Rendered UI verification against the launched flow — **DONE via Playwright (system Chrome) against the full running stack.** Seeded a launched "FY26 Annual Planning" campaign (3 strategic pillars, 5 participants across 2 frozen approvers) and drove the browser as a manager (`performance.objective.team.manage` + `performance.strategic.view` + `performance.cycle.*`): verified the manager Team-objectives cockpit (frozen scope of 3 Consulting participants, strategy pillars, quiet "available when planning opens Jul 25, 2026" note, no lifecycle labels), created two team objectives through the dialog (measurement as bounded choice from the frozen snapshot; saved reflects immediately with toast; nested under pillars with edit/delete + measurement badges), the Direction Strategy door + live cascade coverage (2/3 pillars translated, 1/2 managers contributing, 2 objectives; gaps informational, never blocking), and HR reaching the same read-only coverage inside the launched campaign detail. Verified fail-closed sidebar gating for a PlatformAdmin with no performance permissions (only Overview + Performance configuration; no Team objectives/Strategy/Campaigns). Screenshots in scratchpad `shots/`.
- [x] 6.3 Verify every delta-spec scenario is covered by a test or the rendered-UI check; update `openspec` change status and prepare progressive commits per `.local-docs/git-conventions` (leave push/PR to the user)
