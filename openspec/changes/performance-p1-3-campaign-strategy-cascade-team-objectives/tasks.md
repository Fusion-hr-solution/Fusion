...
- [ ] 6.2 Rendered UI verification against the launched flow: seed a launched campaign, verify manager door (scope, create/edit/delete, empty states), HR coverage in campaign detail, Direction door with strategic-view-only account, schedule-gated wording, and no lifecycle labels anywhere; verify sidebar gating for a user with none of the permissions — **PARTIAL:** sidebar gating covered by unit tests (none/team-manage/strategic-view); all four new routes SSR-render via `next build`. Interactive browser walkthrough against a seeded launched campaign still needs the full stack running + Core workforce data (no Playwright/browser this session) — left for the user's running environment, same as P1.2.
