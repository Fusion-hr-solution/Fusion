...
- [ ] 7.7 Run an `impeccable` craft pass (constrained by Fusion repo rules) across the population → readiness → launch arc: give the pivotal moments (explicit all-active baseline, ready-to-launch state, the launch commit, the launched baseline) deliberate presence and hierarchy while keeping editing/scanning calm; confirm one system + one voice via `@repo/ds` and `campaign-terminology.ts`; strip any clutter, decorative text, or bold that doesn't earn its place.
...
- [ ] 9.2 Rendered browser walkthrough (Playwright): sign in → open a complete Draft campaign → set population (org-unit scope + one exclusion with reason) → open readiness → override a missing-manager participant's approver → launch before the opening date → confirm launched read-only baseline; capture screenshots.
- [ ] 9.3 Verify launch semantics in the rendered app: launch succeeds with a future opening date, the launched campaign shows employee entry as opening on the schedule (not opened by launch), and Draft edits are no longer offered.
- [ ] 9.4 Verify sidebar lifecycle state and campaign breadcrumb render correctly for Draft and Launched campaigns.
- [ ] 9.5 Inspect the rendered UI for *felt* quality, not just correctness: the launch moment reads as decisive and the Draft→Launched change is satisfying and legible; pivotal states have earned presence while editing/scanning stay calm; character is consistent with `@repo/ds` and the terminology voice (one hand), with no decoration or clutter.
- [ ] 9.6 Run `openspec verify --change performance-p1-2-objective-planning-population-launch` (or `/opsx:verify`) and reconcile any gaps before archiving.
